...
This should not be run directly. Instead, navigate to the oppia-ml/ folder and
execute:
...
TEST_RUNNER_PATH = os.path.join(os.getcwd(), 'core', 'tests', 'test_suite.py')
...
def _get_all_test_targets(test_path=None):
    ...
    for root in os.listdir(base_path):
        ...
        for subroot, _, files in os.walk(os.path.join(base_path, root)):
            for f in files:
                if (f.endswith('_test.py') or f.endswith('Test.py') and
